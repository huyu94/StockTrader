...
def run(provider: str, update: bool, concepts: str = None, industries: str = None, limit: int = None, strategy_name: str = "少妇战法"):
    ...
    selected = []
    for ts_code in tqdm(codes, desc="筛选股票", unit="只"):
    ...
    filename_prefix = f"recommend_{provider}_{datetime.now().strftime('%Y%m%d')}"
    output.print_result([{
        "ts_code": s["ts_code"],
        "name": "",
        "industry": "",
    ...
    paths = output.save_result(selected, formats=["csv", "json"], filename_prefix=filename_prefix)
    print("\n输出文件：")
    for p in paths:
        print(p)
...
if __name__ == "__main__":
    ...
    run(args.provider, args.update, None, args.industries, args.limit, args.strategy)
