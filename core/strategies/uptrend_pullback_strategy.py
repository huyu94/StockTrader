"""
上升趋势回调买入策略 (Uptrend Pullback Strategy)

策略名称：上升趋势回调买入
策略逻辑：
1. 均线多头排列：MA20 > MA30 > MA60（确认上升趋势）
2. KDJ指标的J值必须小于等于5（超卖信号，回调到位）
3. 当前成交量必须小于前20日成交量最大值的50%（缩量回调）
4. 收盘价接近MA20且未跌破（在MA20的±3%范围内，且未跌破MA20）

买入条件：
- 同时满足以上四个条件时，认为符合买入条件
- 这是一个寻找上升趋势中回调买入点的策略，适合捕捉趋势中的回调机会
"""
...
class UptrendPullbackStrategy(BaseStrategy):
    ...
    策略说明：
    1. 均线多头排列：MA20 > MA30 > MA60，确保股票处于上升趋势
    2. KDJ指标的J值 <= 5：表示股票处于超卖状态，回调到位
    3. 成交量缩量：当前成交量 < 前20日最大成交量的50%，表示抛压减轻
    4. 价格接近MA20且未跌破：收盘价在MA20的±3%范围内，且未跌破MA20
    ...
    def __init__(
        ...
        Args:
            ...
            vol_shrink_ratio: 成交量缩量比例，默认0.5（50%），当前成交量必须小于前N日最大成交量的此比例
            ma_tolerance: 接近均线的容忍度，默认0.03（3%），表示价格在均线的±3%范围内算接近
            name: 策略名称，默认"上升趋势回调买入"
        """
        super().__init__(name=name or "上升趋势回调买入")
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for ts_code, group_df in df_copy.groupby('ts_code'):
            ...
            # 3. 计算成交量相关指标
            # 计算前N日的成交量最大值
            group_df['vol_max'] = group_df['vol'].rolling(window=self.vol_period).max()
            # 计算当前成交量与最大值的比例
            group_df['vol_ratio'] = np.where(
                group_df['vol_max'] > 0,
                group_df['vol'] / group_df['vol_max'],
                1.0  # 如果最大成交量为0，比例设为1
            )
            
            # 标记是否满足成交量缩量条件（当前成交量 < 最大成交量的指定比例）
            group_df['vol_shrink'] = group_df['vol_ratio'] < self.vol_shrink_ratio
            
            # 标记是否满足J值条件（J值 <= 阈值）
            group_df['kdj_oversold'] = group_df['kdj_j'] <= self.j_threshold
            
            # 4. 计算均线多头排列指标
            # 判断是否满足多头排列：MA20 > MA30 > MA60
            group_df['ma20_gt_ma30'] = group_df['ma20'] > group_df['ma30']
            group_df['ma30_gt_ma60'] = group_df['ma30'] > group_df['ma60']
            group_df['uptrend'] = group_df['ma20_gt_ma30'] & group_df['ma30_gt_ma60']
            ...
            # 判断是否接近MA20且未跌破（在容忍度范围内，且在上方）
            # 接近MA20：在MA20上方 且 偏离度 <= tolerance
            group_df['near_ma20'] = (
                (group_df['close'] > group_df['ma20']) &  # 在MA20上方
                (group_df['close_to_ma20'] <= self.ma_tolerance) &  # 偏离度 <= tolerance
    ...
    def filter_stocks(self, df: pd.DataFrame) -> Union[List[str], pd.DataFrame]:
        ...
        买入条件：
        1. 均线多头排列：MA20 > MA30 > MA60（上升趋势）
        2. KDJ的J值 <= 阈值（回调到位）
        3. 当前成交量 < 前N日最大成交量的指定比例（缩量回调）
        4. 收盘价接近MA20且未跌破（价格在MA20的±3%范围内，且未跌破MA20）
        ...
        debug_stats = {
            ...
            'condition1_fail': 0,  # 多头排列
            'condition2_fail': 0,  # KDJ_J
            'condition3_fail': 0,  # 成交量缩量
            'condition4_fail': 0,  # 价格接近MA20
            'passed': 0
        }
        ...
        for ts_code, group_df in df.groupby('ts_code'):
            ...
            if pd.isna(latest_row.get('vol_max')):
                missing_indicators.append('vol_max')
            ...
            # 条件1：均线多头排列 MA20 > MA30 > MA60
            ma20 = latest_row.get('ma20', 0)
            ma30 = latest_row.get('ma30', 0)
            ma60 = latest_row.get('ma60', 0)
            uptrend = latest_row.get('uptrend', False)
            condition1 = bool(uptrend) if not pd.isna(uptrend) else False
            
            # 条件2：KDJ的J值 <= 阈值（回调到位）
            j_value = latest_row.get('kdj_j', 999)
            condition2 = j_value <= self.j_threshold if not pd.isna(j_value) else False
            
            # 条件3：当前成交量 < 前N日最大成交量的指定比例（缩量回调）
            vol_ratio = latest_row.get('vol_ratio', 1.0)
            condition3 = vol_ratio < self.vol_shrink_ratio if not pd.isna(vol_ratio) else False
            
            # 条件4：收盘价接近MA20且未跌破
            near_ma20 = latest_row.get('near_ma20', False)
            condition4 = bool(near_ma20) if not pd.isna(near_ma20) else False
            ...
            if condition1 and condition2 and condition3 and condition4:
                ...
                close_to_ma20 = latest_row.get('close_to_ma20', 0)
