...
我们已经对项目结构进行了全面整理，主要包括：

1. **文件整理**
   - 将根目录下的测试文件移动到 `tests` 目录
   - 将根目录下的示例文件移动到 `examples` 目录
   - 清理了 `scripts` 目录，只保留实际的脚本文件
   - 删除了不必要的 `trade_calendar_2025.csv` 文件
   - 将可视化相关代码移动到 `src/visualization` 目录
...
现在项目结构更加清晰，文件都在正确的位置，便于后续开发和维护。
