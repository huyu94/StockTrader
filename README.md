# 股票分析系统

## 项目概述

基于Tushare接口的股票分析系统，实现股票数据获取、技术指标计算、策略筛选和数据可视化功能。

本项目采用**双架构设计**：
1. **传统架构**（`src/`）：Provider → Fetcher → Storage → Manager 分层架构，用于快速数据获取
2. **ETL Pipeline 架构**（`core/`）：Collector → Transformer → Loader 的 ETL 流水线架构，用于标准化数据处理流程

---

## 一、ETL Pipeline 架构（核心架构）

### 1.1 架构概述

系统采用 **ETL Pipeline 架构**，将数据处理流程分为三个独立阶段：

```
┌─────────────┐     ┌──────────────┐     ┌────────────┐
│  Collector  │ --> │ Transformer  │ --> │   Loader   │
└─────────────┘     └──────────────┘     └────────────┘
      ↓                    ↓                    ↓
   数据采集             数据清洗转换          数据入库
```

### 1.2 核心组件

#### Collectors（数据采集层）

**职责**：从外部数据源采集原始数据

- ✅ `BaseCollector` - 采集器基类
- ✅ `DailyKlineCollector` - 日K线数据采集
- ✅ `AdjFactorCollector` - 复权因子采集
- ✅ `BasicInfoCollector` - 股票基本信息采集
- ✅ `TradeCalendarCollector` - 交易日历采集
- ✅ `ExDateCollector` - 除权除息日采集（内部组件）

**特性**：
- 支持多数据源切换（Tushare、Akshare等）
- 内置重试机制
- 参数验证
- 统一的数据返回格式（DataFrame）

#### Transformers（数据转换层）

**职责**：数据清洗、标准化、衍生指标计算

- ✅ `BaseTransformer` - 转换器基类
- ✅ `DailyKlineTransformer` - K线数据清洗、标准化
- ✅ `AdjFactorTransformer` - 复权因子处理
- ✅ `BasicInfoTransformer` - 基本信息处理
- ✅ `TradeCalendarTransformer` - 交易日历处理

**主要功能**：
- 字段重命名和映射
- 数据类型转换
- 异常值处理
- 缺失值处理
- 数据质量验证

#### Loaders（数据加载层）

**职责**：将处理后的数据持久化到数据库

- ✅ `BaseLoader` - 加载器基类
- ✅ `DailyKlineLoader` - K线入库逻辑
- ✅ `AdjFactorLoader` - 复权因子入库逻辑
- ✅ `BasicInfoLoader` - 基本信息入库逻辑
- ✅ `TradeCalendarLoader` - 交易日历入库逻辑

**加载策略**：
- `append`：追加数据（使用 INSERT IGNORE）
- `replace`：替换数据（先删除再插入）
- `upsert`：存在则更新，不存在则插入（使用 INSERT ... ON DUPLICATE KEY UPDATE）

**特性**：
- 支持批量加载（可配置批量大小）
- 支持 MySQL 数据库（通过 SQLAlchemy ORM）
- 完善的错误处理和事务管理
- 数据验证（加载前验证必需列）

#### Models（数据模型）

**业务数据模型**：
- ✅ `DailyKline` - 日K线数据模型
- ✅ `AdjFactor` - 复权因子数据模型
- ✅ `StockBasicInfo` - 股票基本信息模型
- ✅ `TradeCalendar` - 交易日历模型

**ORM 模型**：
- ✅ `DailyKlineORM` - 日K线 ORM 模型
- ✅ `AdjFactorORM` - 复权因子 ORM 模型
- ✅ `BasicInfoORM` - 基本信息 ORM 模型
- ✅ `TradeCalendarORM` - 交易日历 ORM 模型

所有 ORM 模型统一在 `core/models/orm.py` 中定义。

#### Pipelines（流水线编排层）

**职责**：编排 ETL 流程

- ✅ `BasePipeline` - Pipeline 基类（框架已搭建）
- ⏳ `DailyPipeline` - 每日更新流水线（待实现）
- ⏳ `HistoryPipeline` - 历史数据补全流水线（待实现）
- ⏳ `RealtimePipeline` - 实时数据流水线（待实现）

#### Orchestrator（调度编排层）

**职责**：任务调度、依赖管理、监控告警

- ⏳ `TaskScheduler` - 任务调度器（框架已搭建，待实现）
- ⏳ `DependencyManager` - 任务依赖管理（框架已搭建，待实现）
- ⏳ `TaskMonitor` - 监控和告警（框架已搭建，待实现）

#### Common（公共组件）

- ✅ `exceptions.py` - 异常定义（已完成）
- ⏳ `validators.py` - 数据验证（框架已搭建，待实现）
- ⏳ `utils.py` - 工具函数（框架已搭建，待实现）

### 1.3 目录结构

```
core/
├── collectors/              # 数据采集层 ✅
│   ├── base.py
│   ├── daily_kline.py
│   ├── adj_factor.py
│   ├── ex_date.py
│   ├── basic_info.py
│   └── trade_calendar.py
│
├── transformers/            # 数据转换层 ✅
│   ├── base.py
│   ├── daily_kline.py
│   ├── adj_factor.py
│   ├── basic_info.py
│   └── trade_calendar.py
│
├── loaders/                 # 数据加载层 ✅
│   ├── base.py
│   ├── daily_kline.py
│   ├── adj_factor.py
│   ├── basic_info.py
│   └── trade_calendar.py
│
├── pipelines/               # 流水线编排层 ⏳
│   ├── base.py
│   ├── daily_pipeline.py
│   ├── history_pipeline.py
│   └── realtime_pipeline.py
│
├── models/                  # 数据模型 ✅
│   ├── daily_kline.py      # 业务数据模型
│   ├── adj_factor.py
│   ├── stock_basic_info.py
│   ├── calendar.py
│   └── orm.py              # ORM 模型
│
├── orchestrator/            # 调度编排层 ⏳
│   ├── scheduler.py
│   ├── dependency.py
│   └── monitor.py
│
├── config/                  # 配置管理 📋
│   ├── pipelines/          # Pipeline配置（示例文件）
│   ├── schedules.yaml.example
│   ├── sources.yaml.example
│   └── database.yaml.example
│
└── common/                  # 公共组件 ⏳
    ├── exceptions.py       # ✅ 已完成
    ├── validators.py       # ⏳ 待实现
    └── utils.py            # ⏳ 待实现
```

### 1.4 项目进度

#### ✅ 已完成（约 70%）

1. **数据采集层（Collectors）** - 100%
   - 所有采集器已实现
   - 支持 Tushare 数据源
   - 包含重试机制和错误处理

2. **数据转换层（Transformers）** - 100%
   - 所有转换器已实现
   - 支持数据清洗、标准化、验证

3. **数据加载层（Loaders）** - 100%
   - 所有加载器已实现
   - 支持三种加载策略（append/replace/upsert）
   - 支持 MySQL 数据库
   - 支持批量加载

4. **数据模型（Models）** - 100%
   - 业务数据模型已实现
   - ORM 模型已实现并统一管理

5. **公共组件（Common）** - 50%
   - 异常定义已完成
   - 验证器和工具函数待实现

#### ⏳ 待实现（约 30%）

1. **流水线编排层（Pipelines）** - 0%
   - 框架已搭建
   - 需要实现具体的 ETL 流程编排

2. **调度编排层（Orchestrator）** - 0%
   - 框架已搭建
   - 需要实现任务调度、依赖管理、监控告警

3. **配置管理** - 0%
   - 示例配置文件已提供
   - 需要实现配置加载和解析

4. **工具函数** - 0%
   - 需要实现 DataFrame 合并、分块等工具函数

### 1.5 使用示例

#### 基本使用

```python
from core.loaders.daily_kline import DailyKlineLoader
from core.transformers.daily_kline import DailyKlineTransformer
from core.collectors.daily_kline import DailyKlineCollector

# 创建组件
collector = DailyKlineCollector(config)
transformer = DailyKlineTransformer(config)
loader = DailyKlineLoader(config)

# 执行 ETL 流程
raw_data = collector.collect(params)
clean_data = transformer.transform(raw_data)
loader.load(clean_data)
```

#### 配置示例

```yaml
# config/pipelines/daily_kline.yaml
daily_kline:
  collector:
    source: tushare
    token: ${TUSHARE_TOKEN}
    retry_times: 3
    
  transformer:
    transform_rules:
      remove_halted: true
      validate_ohlc: true
      
  loader:
    table: daily_kline
    load_strategy: upsert
    upsert_keys: ['ts_code', 'trade_date']
    batch_size: 1000
```

---

## 二、传统架构（兼容架构）

### 2.1 架构概览

系统采用分层架构设计，从底层到顶层分为：**Provider层** → **Fetcher层** → **Storage层** → **Manager层**。

```
┌─────────────────────────────────────────────────────────────┐
│                      Manager (统一管理器)                     │
│  - 协调数据获取流程                                           │
│  - 管理线程池（IO线程池、任务线程池）                          │
│  - 智能更新策略（全量更新/增量更新）                           │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│                    Fetcher (数据获取层)                       │
│  - DailyKlineFetcher: 日线行情获取                           │
│  - BasicInfoFetcher: 股票基本信息获取                         │
│  - CalendarFetcher: 交易日历获取                              │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│                    Provider (API接口层)                       │
│  - TushareProvider: Tushare API封装                          │
│    • 单例模式，确保API调用串行                                │
│    • 使用锁机制避免IP超限                                     │
│    • 支持 pro_bar API（快速批量获取）                          │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│                    Storage (数据存储层)                        │
│  - DailyKlineStorageMySQL: 日线数据MySQL存储                 │
│  - BasicInfoStorageMySQL: 基本信息MySQL存储                    │
│  - CalendarStorageMySQL: 交易日历MySQL存储                   │
│    • 统一使用MySQL数据库，支持连接池                          │
│    • 支持批量写入和UPSERT操作                                 │
└─────────────────────────────────────────────────────────────┘
```

### 2.2 核心组件详解

#### 1. Provider层 (`src/providers/`)

**TushareProvider** - Tushare API封装类

- **单例模式**：确保全局只有一个API实例
- **串行调用**：使用 `threading.Lock` 确保所有API调用完全串行，避免IP超限
- **性能优化**：
  - 设置 `NO_PROXY` 环境变量，绕过系统代理提高速度
  - 使用 `pro_bar` API，一次调用获取单只股票全部历史数据
  - 支持 `factors` 参数，同时获取复权因子

**关键方法**：
- `pro_bar()`: 使用 pro_bar API 获取K线数据（推荐，更快）
- `query()`: 通用查询接口
- `daily()`: 使用 daily API 获取日线数据（用于按日期批量获取）

#### 2. Fetcher层 (`src/fetchers/`)

**DailyKlineFetcher** - 日线行情获取器

- `fetch_one()`: 获取单只股票的日线数据
  - 使用 `pro_bar` API，一次获取全部历史
  - 自动获取复权因子（`factors="tor"`）
  - 统一列名处理（`factor` → `adj_factor`）

- `fetch_daily_by_date()`: 按日期获取全市场数据
  - 用于增量更新场景
  - 当某个交易日缺失数据较多时使用
  - 使用 `daily` API（pro_bar不支持按日期获取全市场数据）

**其他Fetcher**：
- `BasicInfoFetcher`: 获取股票基本信息
- `CalendarFetcher`: 获取交易日历
- `AdjFactorFetcher`: 复权因子获取（已弃用，现包含在日线数据中）

#### 3. Storage层 (`src/storage/`)

**MySQL存储架构**

所有数据统一存储在 MySQL 数据库中，包含以下表：

- `daily_kline`: 日线行情数据（包含复权因子）
- `adj_factor`: 复权因子数据（保留，但已包含在daily_kline中）
- `basic_info`: 股票基本信息
- `trade_calendar`: 交易日历

**性能优势**：
- **批量写入**：单次事务写入所有数据，性能提升5-15倍
- **自动去重**：使用 `INSERT ... ON DUPLICATE KEY UPDATE` 自动处理重复数据
- **索引优化**：为常用查询字段创建索引
- **连接池管理**：使用 SQLAlchemy 连接池，提升并发性能

**MySQLBaseStorage** - 存储基类
- 提供统一的数据库连接管理
- 实现 `_bulk_upsert_dataframe`，支持批量UPSERT操作
- 自动处理NaN值转换

#### 4. Manager层 (`src/manager.py`)

**Manager** - 统一数据管理器

**核心功能**：
- 统一管理所有数据类型（日线、复权因子、基础信息、日历）
- 维护线程池：
  - `io_executor`: 20个工作线程，用于密集文件IO操作
  - `task_executor`: 1个工作线程，用于后台任务调度

**智能更新策略**：

1. **全量更新（按股票代码）** - `_update_all_stocks_full()`
   - 适用场景：首次爬取数据
   - 流程：遍历所有股票 → 获取最近一年数据 → 覆盖写入
   - 优势：数据完整，适合初始化

2. **增量更新（按交易日）** - `_update_missing_data_incremental()`
   - 适用场景：定期更新数据
   - 流程：
     - 生成数据存在性矩阵（交易日 × 股票代码）
     - 识别缺失数据较多的交易日（阈值：1000只股票）
     - 批量获取该日所有股票数据
     - 批量写入数据库
   - 优势：只更新缺失数据，效率高

### 2.3 项目结构

```
.
├── src/
│   ├── providers/          # API接口层
│   │   ├── base_provider.py
│   │   └── tushare_provider.py
│   ├── fetch/             # 数据获取层
│   │   ├── daily_kline/
│   │   ├── basic_info/
│   │   ├── calendar/
│   │   └── adj_factor/
│   ├── storage/            # 数据存储层
│   │   ├── mysql_base.py
│   │   ├── daily_kline_storage_mysql.py
│   │   ├── adj_factor_storage_mysql.py
│   │   ├── basic_info_storage_mysql.py
│   │   ├── calendar_storage_mysql.py
│   │   └── orm_models.py   # 向后兼容层（已迁移到 core/models/orm.py）
│   ├── manager.py          # 统一管理层
│   └── strategy/           # 策略模块
│
├── core/                   # ETL Pipeline 架构（核心架构）
│   ├── collectors/         # 数据采集层 ✅
│   ├── transformers/       # 数据转换层 ✅
│   ├── loaders/            # 数据加载层 ✅
│   ├── pipelines/          # 流水线编排层 ⏳
│   ├── models/             # 数据模型 ✅
│   ├── orchestrator/       # 调度编排层 ⏳
│   ├── config/             # 配置管理 📋
│   └── common/             # 公共组件 ⏳
│
├── scripts/
│   ├── run_fetch.py        # 数据爬取脚本
│   ├── init_sqlite_db.py   # 数据库初始化脚本
│   └── view_sqlite_data.py # 数据查看脚本
│
└── data/
    └── stock_data.db       # SQLite数据库文件（可选）
```

---

## 三、环境准备

### 3.1 安装依赖

```bash
uv install
```

### 3.2 配置环境变量

创建 `.env` 文件：

```env
# Tushare API密钥
TUSHARE_TOKEN=your_tushare_token_here

# MySQL 配置（用于 ETL Pipeline 架构）
MYSQL_HOST=localhost
MYSQL_PORT=3306
MYSQL_USER=root
MYSQL_PASSWORD=your_password
MYSQL_DATABASE=stock_data
MYSQL_CHARSET=utf8mb4

# 数据存储路径（可选）
DATA_PATH=data/
```

### 3.3 初始化数据库

```bash
# 初始化 MySQL 数据库表（ETL Pipeline 架构）
# 表结构会自动创建（通过 ORM 模型）

# 或初始化 SQLite 数据库（传统架构）
uv run scripts/init_sqlite_db.py
```

---

## 四、技术栈

- **Python 3.8+**
- **Tushare Pro API**: 股票数据源
- **MySQL**: 数据存储（ETL Pipeline 架构）
- **SQLite**: 数据存储（传统架构，可选）
- **SQLAlchemy**: ORM 框架
- **Pandas**: 数据处理
- **Loguru**: 日志管理
- **uv**: Python 包管理工具

---

## 五、关键特性

### ETL Pipeline 架构特性

1. **模块化设计**：Collector、Transformer、Loader 三层独立，易于扩展
2. **配置驱动**：通过 YAML 配置文件控制数据处理流程
3. **多种加载策略**：支持 append、replace、upsert 三种数据加载方式
4. **批量处理**：支持批量数据加载，提升性能
5. **数据验证**：加载前自动验证数据完整性和正确性
6. **错误处理**：完善的异常处理和日志记录

### 传统架构特性

1. **智能更新策略**：自动选择全量更新或增量更新
2. **高性能存储**：MySQL 批量写入，性能提升5-15倍
3. **API优化**：使用 pro_bar API，一次获取全部历史数据
4. **复权因子集成**：复权因子已包含在日线数据中，无需单独爬取
5. **线程安全**：使用锁机制确保API调用串行，避免IP超限

---

## 六、使用方式

### ETL Pipeline 架构使用

```python
from core.collectors.daily_kline import DailyKlineCollector
from core.transformers.daily_kline import DailyKlineTransformer
from core.loaders.daily_kline import DailyKlineLoader

# 配置
config = {
    "collector": {
        "source": "tushare",
        "token": "your_token"
    },
    "transformer": {
        "transform_rules": {
            "remove_halted": True,
            "validate_ohlc": True
        }
    },
    "loader": {
        "table": "daily_kline",
        "load_strategy": "upsert",
        "upsert_keys": ["ts_code", "trade_date"],
        "batch_size": 1000
    }
}

# 创建组件
collector = DailyKlineCollector(config["collector"])
transformer = DailyKlineTransformer(config["transformer"])
loader = DailyKlineLoader(config["loader"])

# 执行 ETL
params = {"ts_code": "000001.SZ", "start_date": "20240101", "end_date": "20241231"}
raw_data = collector.collect(params)
clean_data = transformer.transform(raw_data)
loader.load(clean_data)
```

### 传统架构使用

```bash
# 全量更新（按股票代码，首次爬取）
uv run scripts/run_fetch.py --mode code --lookback-days 365

# 增量更新（按交易日，定期更新，默认）
uv run scripts/run_fetch.py --mode date --lookback-days 365
```

---

## 七、注意事项

1. **API限制**：Tushare API有调用频率限制，系统已通过串行调用避免超限
2. **数据完整性**：首次爬取建议使用全量更新
3. **定期更新**：建议每日使用增量更新
4. **数据库备份**：建议定期备份数据库
5. **架构选择**：
   - 新项目推荐使用 **ETL Pipeline 架构**（`core/`），更标准化、易维护
   - 传统架构（`src/`）保留用于兼容和快速数据获取

---

## 八、架构设计决策

### 8.1 前复权计算的架构设计

#### 问题背景

前复权计算需要依赖两个数据源：
- 日K线数据（来自 `DailyKlineCollector`）
- 复权因子数据（来自 `AdjFactorCollector`）

如果放在 Transformer 层，会导致：
- Transformer 依赖其他 Collector/Loader（违反单一职责原则）
- 跨层依赖（Transformer 不应该知道数据来源）
- 难以测试（需要 mock 多个依赖）

#### 解决方案：Pipeline 层协调 + 独立计算器模块

**架构设计**：

```
Pipeline 层（编排）：
  ├─ 协调多个 Collector
  │   ├─ DailyKlineCollector.collect() -> 日K线数据
  │   └─ AdjFactorCollector.collect() -> 复权因子数据
  ├─ 传递数据给 Calculator
  └─ 管理数据流

Calculator 层（计算）：
  ├─ QFQCalculator.calculate(kline_df, adj_factor_df)
  ├─ 接收所需数据（通过参数传入，无隐藏依赖）
  └─ 可选：从 Loader 读取辅助数据（同层依赖，可接受）

Transformer 层（转换）：
  └─ 只负责数据清洗和标准化，不涉及复杂计算
```

**依赖关系**：

```
Pipeline
  ├─> DailyKlineCollector
  ├─> AdjFactorCollector
  ├─> QFQCalculator (接收两个 Collector 的数据)
  ├─> DailyKlineTransformer (只接收日K线数据)
  └─> DailyKlineLoader

QFQCalculator
  ├─ 接收参数：kline_df, adj_factor_df
  └─ [可选] 依赖 Loader 读取复权因子（同层依赖，可接受）
```

**设计原则**：

1. **单一职责**：每层只做自己的事
   - Pipeline：负责编排和协调
   - Calculator：负责计算逻辑
   - Transformer：负责数据清洗

2. **依赖倒置**：依赖抽象（DataFrame），不依赖具体实现
   - Calculator 接收 DataFrame 参数，不直接调用 Collector

3. **开闭原则**：易于扩展，无需修改现有代码
   - 新增计算器只需实现接口，无需修改 Pipeline

**实现示例**：

```python
# Pipeline 层协调
class DailyPipeline(BasePipeline):
    def run(self, stock_codes: List[str], **kwargs):
        # 1. 获取日K线数据
        kline_data = self.kline_collector.collect(params)
        
        # 2. 获取复权因子数据
        adj_factor_data = self.adj_factor_collector.collect(params)
        
        # 3. 数据清洗
        clean_data = self.transformer.transform(kline_data)
        
        # 4. 计算前复权（可选）
        if self.config.get("calculate_qfq", False):
            qfq_calculator = QFQCalculator()
            clean_data = qfq_calculator.calculate(clean_data, adj_factor_data)
        
        # 5. 入库
        self.loader.load(clean_data)
```

### 8.2 分时股价数据的架构设计

#### 数据特点

分时股价数据具有以下特点：
- **数据量大**：一天可能有几百条记录（每分钟或每5分钟一条）
- **实时性要求高**：需要及时更新当日数据
- **需要与日K线结合**：可能用于补充或验证当日K线数据
- **时间序列特性**：按时间顺序存储，需要高效的时间范围查询

#### 架构设计

**方案：独立的分时数据模块 + 聚合计算器**

```
core/
├── collectors/
│   └── intraday_kline.py    # ✨ 分时数据采集器
│
├── transformers/
│   └── intraday_kline.py    # ✨ 分时数据转换器
│
├── loaders/
│   └── intraday_kline.py    # ✨ 分时数据加载器
│
├── calculators/
│   ├── qfq_calculator.py    # 前复权计算器
│   └── aggregator.py        # ✨ 聚合计算器（分时 -> 日K线）
│
├── models/
│   └── intraday_kline.py    # ✨ 分时数据模型
│
└── pipelines/
    ├── daily_pipeline.py    # 日K线流水线
    └── intraday_pipeline.py # ✨ 分时数据流水线
```

**数据流程**：

```
1. 分时数据采集
   IntradayKlineCollector.collect()
   ↓
2. 分时数据转换
   IntradayKlineTransformer.transform()
   ↓
3. 分时数据入库
   IntradayKlineLoader.load()
   ↓
4. [可选] 聚合为日K线
   Aggregator.aggregate_to_daily()
   ↓
5. 与历史日K线合并/验证
   DailyKlineLoader.load()
```

**关键设计点**：

1. **独立存储**：分时数据单独存储，不混入日K线表
   - 表名：`intraday_kline`
   - 主键：`(ts_code, trade_date, time)` 或 `(ts_code, datetime)`
   - 索引：`ts_code`, `trade_date`, `datetime`

2. **聚合计算器**：将分时数据聚合为日K线
   - 开盘价：当日第一笔交易价格
   - 收盘价：当日最后一笔交易价格
   - 最高价：当日所有分时数据的最高价
   - 最低价：当日所有分时数据的最低价
   - 成交量：当日所有分时数据的成交量之和

3. **数据结合策略**：
   - **方案A**：分时数据作为日K线的补充
     - 如果日K线数据缺失，从分时数据聚合生成
     - 如果日K线数据存在，用于验证和修正
   
   - **方案B**：分时数据独立使用
     - 分时数据用于实时分析和策略
     - 日K线数据用于历史回测和分析
     - 两者互不干扰

4. **实时更新策略**：
   - **增量更新**：只更新当日的分时数据
   - **定时更新**：交易时间内定期更新（如每5分钟）
   - **收盘后聚合**：收盘后将当日分时数据聚合为日K线

**实现示例**：

```python
# 分时数据模型
class IntradayKline:
    ts_code: str
    trade_date: str  # YYYY-MM-DD
    time: str        # HH:MM:SS
    datetime: datetime  # 完整时间戳
    price: float     # 价格
    volume: int      # 成交量
    amount: float    # 成交额

# 聚合计算器
class Aggregator:
    def aggregate_to_daily(self, intraday_df: pd.DataFrame) -> pd.DataFrame:
        """将分时数据聚合为日K线"""
        # 按日期分组
        daily_data = intraday_df.groupby(['ts_code', 'trade_date']).agg({
            'price': ['first', 'last', 'max', 'min'],  # 开、收、高、低
            'volume': 'sum',
            'amount': 'sum'
        })
        # ... 处理逻辑
        return daily_df

# 分时数据流水线
class IntradayPipeline(BasePipeline):
    def run(self, stock_codes: List[str], trade_date: str):
        # 1. 采集分时数据
        intraday_data = self.collector.collect(stock_codes, trade_date)
        
        # 2. 转换
        clean_data = self.transformer.transform(intraday_data)
        
        # 3. 入库
        self.loader.load(clean_data)
        
        # 4. [可选] 聚合为日K线
        if self.config.get("aggregate_to_daily", False):
            aggregator = Aggregator()
            daily_data = aggregator.aggregate_to_daily(clean_data)
            # 更新日K线数据
            self.daily_loader.load(daily_data)
```

**与日K线结合使用**：

```python
# 在日K线 Pipeline 中使用分时数据
class DailyPipeline(BasePipeline):
    def run(self, stock_codes: List[str], trade_date: str):
        # 1. 尝试从日K线获取数据
        daily_data = self._load_daily_kline(stock_codes, trade_date)
        
        # 2. 如果日K线数据缺失，从分时数据聚合
        missing_codes = self._find_missing_codes(daily_data, stock_codes)
        if missing_codes:
            intraday_data = self._load_intraday_kline(missing_codes, trade_date)
            aggregated_data = self.aggregator.aggregate_to_daily(intraday_data)
            daily_data = pd.concat([daily_data, aggregated_data])
        
        # 3. 继续后续处理
        clean_data = self.transformer.transform(daily_data)
        self.loader.load(clean_data)
```

**存储优化**：

1. **分区存储**：按日期分区，提高查询效率
2. **数据保留策略**：只保留最近N天的分时数据，历史数据可归档
3. **索引优化**：为常用查询字段创建索引（ts_code, trade_date, datetime）

---

## 九、项目进度总结

### 总体进度：约 70%

- ✅ **数据采集层（Collectors）** - 100%
- ✅ **数据转换层（Transformers）** - 100%
- ✅ **数据加载层（Loaders）** - 100%
- ✅ **数据模型（Models）** - 100%
- ⏳ **流水线编排层（Pipelines）** - 0%（框架已搭建）
- ⏳ **调度编排层（Orchestrator）** - 0%（框架已搭建）
- ⏳ **配置管理** - 0%（示例文件已提供）
- ⏳ **公共组件（部分）** - 50%（异常定义已完成）
- ⏳ **计算器模块（Calculators）** - 0%（架构已设计）
- ⏳ **分时数据模块（Intraday）** - 0%（架构已设计）

### 下一步计划

1. 实现 Pipelines 层的具体 ETL 流程编排
2. 实现 Orchestrator 层的任务调度和依赖管理
3. 完善配置管理系统
4. 实现公共工具函数
5. 实现计算器模块（前复权计算器、聚合计算器）
6. 实现分时数据模块（采集、转换、加载）
7. 编写单元测试和集成测试

---

## 九、许可证

MIT License

## 十、免责声明

本项目仅用于学习和研究，不构成任何投资建议。股票投资有风险，入市需谨慎。
